...
date: "Markov Chain Monte Carlo (MCMC) Diagnostics"
output:
...
A graph of the sampled parameter values as a function of the MCMC step is called a _trace plot_. Here's some examples. Which one do you think is indicating convergence?

```{r, fig.height=2.9,fig.width=5.5, warning=FALSE}
...
theta_cur2 <- as_tibble(mcmc_theta(sigma_pro = 0.2))
...
theta_cur3 <- suppressWarnings(as_tibble(mcmc_theta(sigma_pro = 0.02,
                                                   n_iter = 10000)))
names(theta_cur3) <- paste0("chain",1:3)
theta_cur3$iteration <- 1:10000
theta_cur3 <- theta_cur3 %>% pivot_longer(-iteration,
                          names_to = "chain",
                          values_to = "theta")

p1 <- ggplot(theta_cur3[3001:nrow(theta_cur3),], aes(x = iteration, y = theta, colour = chain)) +
  geom_line() +
  ylab(expression(theta))
p2 <- ggplot(theta_cur3[1:3000,], aes(x = iteration, y = theta, colour = chain)) +
  geom_line()+
  ylab(expression(theta))

suppressMessages(ggpubr::ggarrange(p2,p1, common.legend = TRUE))
```
...
```{r}
theta_cur <- mcmc_theta(sigma_pro = 0.2,
                        n_iter = 10000)

mcmc_list <- mcmc.list(as.mcmc(theta_cur[,1]),
                          as.mcmc(theta_cur[,2]),
                          as.mcmc(theta_cur[,3]))
...
- MCMC samples are NOT independent draws from a target
distribution, because:

    1. The first draw is set by the user and thus not a random draw from
...
We want to know what the sample size of a non-autocorrelated chain, that yields the same information, would be. An answer to this question can be provided with a measure called _effective sample size (ESS)_
